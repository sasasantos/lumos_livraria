...
class Livro(db.Model):
    ...
    idioma_id = db.Column(db.Integer, db.ForeignKey('idioma.id', ondelete='SET NULL'), nullable=True)
...
class ItemVenda(db.Model):
    ...
    livro_id = db.Column(db.Integer, db.ForeignKey('livro.id', ondelete='CASCADE'))
    quantidade = db.Column(db.Integer)

    # subquery evita cartesian product de joins aninhados
    livro = db.relationship('Livro', lazy='subquery')
...
class Venda(db.Model):
    ...
    # subquery: carrega itens em 1 query separada (sem cartesian join com Livro)
